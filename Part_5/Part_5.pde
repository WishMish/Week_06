...
void draw() {
  ...
  for (int i = 0; i < gridW; i++) {
    for (int j = 0; j < gridH; j++) {
      size[i][j] = map(i, 0, 10, 10,50);
      float b = colors[i][j] = map(i, 0,10, 100,200);
      float r = colors[i][j] = map(j, 10,0, 50,200);
      fill(r,0,b);
      ellipse(xPositions[i][j], yPositions[i][j], size[i][j], size[i][j]);
      xPositions[i][j] += dx[i][j];
      yPositions[i][j] += dy[i][j];
      
      if (xPositions[i][j] > width-size[i][j]/2 || xPositions[i][j]< size[i][j]/2){
        dx[i][j] = -dx[i][j];
      }
      if (yPositions[i][j] > width-size[i][j]/2 || yPositions[i][j]< size[i][j]/2){
        dy[i][j] = -dy[i][j];
      }
    }
  }
